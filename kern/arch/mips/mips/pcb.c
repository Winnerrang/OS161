...
// Now we are still in menu stack, sp points to the top of kernel space
void
md_switch(struct pcb *old, struct pcb *nu)
{
	...
	// swap interrupt state of new and old thread
	// e.g yield() which disables interrupt and schedule a wake-up thread whose interrupt remains intact
	// a swap in necessary so that we won't disable our interrupt when we have a thread that is waken up.
