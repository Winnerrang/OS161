...
kill_curthread(u_int32_t epc, unsigned code, u_int32_t vaddr)
{
	 ...
	 */
	// in crash, we will jump to here. However, we don't want to panic
	// we should kill the current thread's in userspace
	// this equal to exit syscall, no reap (as simple as possible as instructed in the handout)
	P(mutex);
	assert(curthread->self_pid != -1);
	//process_table[curthread->self_pid]->exitcode = exitcode;
	process_table[curthread->self_pid]->valid = 0;
...
mips_trap(struct trapframe *tf)
{
	...
	switch (code) {
	case EX_MOD:
		//kprintf("\nthe current sp of process %d is %x\n", curthread->self_pid, tf->tf_sp);
	...
	case EX_DBE:
		 ...
		 */
		panic("Bus error exception, PC=0x%x, fault addr is %x\n", tf->tf_epc, tf->tf_vaddr);
		break;
	}
