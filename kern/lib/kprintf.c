...
kprintf(const char *fmt, ...)
{
	...
	//how to print variable length arg to console
	va_start(ap, fmt);
...
panic(const char *fmt, ...)
{
	 ...
	 */
	static volatile int evil; //initialized to 0 as static
	...
	if (evil==2) {
		...
		// print the error message that is passed as variable argument into the function
		kprintf("panic: ");
		va_start(ap, fmt);
