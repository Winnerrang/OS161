...
{	
	// if not used, set to void for safety
	(void)nargs;
	(void)args;
...
static
int
debug_command(int nargs, char **args){

	// debug number mapping
	int debug_number = atoi(args[1]);
	int array_number_map[] = {0x000,0x001,0x002,0x004,0x008,
    						  0x010,0x020,0x040,0x080,
        					  0x100,0x200,0x400,0x800};
	
	//error handling
	if (nargs != 3 || strcmp(args[0],"df") ||
		debug_number > 12 || debug_number <= 0 ||
		(strcmp(args[2],"on") && strcmp(args[2],"off"))) {
		kprintf("Incorrect Usage!!!\n");
		kprintf("Usage: df nr on/off\n");
		return EINVAL;
	}

	if (!strcmp(args[2],"on")){
		dbflags = dbflags | array_number_map[debug_number];
	}else{
		dbflags = dbflags ^ array_number_map[debug_number];
	}
...
{
	// by not setting them to void, not using them will be prompted 
	// with the warning, which in our compile optons, will be reported
	// as the error
	(void)n;
	(void)a;
...
//
// Command table.
// we can create an an tagless struct, i.e no struct tag
static struct {
	const char *name;
	int (*func)(int nargs, char **args);
} cmdtable[] = { // this is an array of struct
	/* menus */
	{ "?",		cmd_mainmenu },
	{ "h",		cmd_mainmenu },
	{ "help",	cmd_mainmenu },
...
cmd_dispatch(char *cmd)
{
	...
	// strtok_r return the first word that is prior 
	// to the delimiter and update the given string by eliminating
	// the returned word
	...
	// The last entry of cmdtable is null null, which will safely end the loop
	// however, this means no cmd has found
...
menu_execute(char *line, int isargs)
{
	...
	// here we just extract the full command and execute it
